...
async def get_debtors_list(message: types.Message):
    ...
    if len(debtors_list) == 0:
        ...
        debtors_list = sorted(debtors_list, key=lambda x: datetime.now().date() - x.date_expiration, reverse=True)
...
async def get_last_operations(message: types.Message):
    """Последние 10 операций пополнения счетов всх пользователей"""
